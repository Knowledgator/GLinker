    ...
    def _validate_pipeline(self):
        ...
        for method_name, _ in self.pipeline:
            if method_name not in available:
                raise ValueError(
                    f"Method '{method_name}' not found in component. Available: {available}"
                )

    def _execute_pipeline_step(self, data: Any, method_name: str, kwargs: dict[str, Any]) -> Any:
        """Execute single pipeline step."""
        method = getattr(self.component, method_name)
        return method(data, **kwargs)

    def _execute_pipeline(
        self, data: Any, pipeline: list[tuple[str, dict[str, Any]]] | None = None
    ) -> Any:
