...
class ReshapeConfig(BaseModel):
    ...
    mode: Literal["flatten_per_group", "preserve_structure"] = Field(
        "flatten_per_group", description="Reshape mode"
    )
...
class InputConfig(BaseModel):
    ...
    fields: str | List[str] | None = Field(
        None, description="JSONPath fields: 'entities[*].text' or ['label', 'score']"
    )

    reduce: Literal["all", "first", "last", "flatten"] = Field(
        "all", description="Reduction mode for lists"
    )

    reshape: ReshapeConfig | None = Field(None, description="Data reshaping configuration")

    template: str | None = Field(
        None, description="Field concatenation template: '{label}: {description}'"
    )

    filter: str | None = Field(None, description="Filter expression: 'score > 0.5'")
...
class OutputConfig(BaseModel):
    ...
    fields: str | List[str] | None = Field(
        None, description="Fields to save (optional, defaults to all)"
    )
...
class PipeNode(BaseModel):
    ...
    requires: List[str] = Field(
        default_factory=list, description="Explicit dependencies (node IDs)"
    )

    config: Dict[str, Any] = Field(default_factory=dict, description="Processor configuration")

    field_schema: Dict[str, Any] | None = Field(
        None, alias="schema", description="Schema for field mappings/transformations"
    )
...
class FieldResolver:
    ...
    @staticmethod
    def _format_template(data: List[Any] | Any, template: str) -> List[str] | str:
...
class DAGExecutor:
    ...
    def _initialize_processors(self):
        ...
        if self.verbose:
            logger.info("All processors initialized and cached")
    ...
    def clear_databases(self, layer_names: List[str] | None = None) -> Dict[str, bool]:
    ...
    def precompute_embeddings(
        self, target_layers: List[str] | None = None, batch_size: int = 32
    ) -> Dict[str, int]:
        """
        ...
        for node_id, processor in self.processors.items():
            ...
            if hasattr(processor, "component") and hasattr(
                processor.component, "precompute_embeddings"
            ):
    ...
    def setup_l3_cache_writeback(self):
        ...
        for _node_id, processor in self.processors.items():
            if hasattr(processor, "component") and hasattr(
                processor.component, "precompute_embeddings"
            ):
                l2_processor = processor
    ...
    def execute(self, pipeline_input: Any) -> PipeContext:
        ...
        for level_idx, level_nodes in enumerate(execution_levels):
            if self.verbose:
                logger.info(f"\n{'=' * 60}")
                logger.info(
                    f"Level {level_idx + 1}/{len(execution_levels)} ({len(level_nodes)} nodes)"
                )
                logger.info(f"{'=' * 60}")
