...
from .models import L1Config, L1Entity, L1GlinerConfig
...
class L1SpacyComponent(BaseComponent[L1Config]):
    ...
    def extract_entities(self, text: str) -> list[L1Entity]:
        ...
        for ent in doc.ents:
            ...
            left_context, right_context = self._get_context(text, ent.start_char, ent.end_char)
    ...
    def filter_by_length(
        self, entities: list[L1Entity], min_length: int | None = None
    ) -> list[L1Entity]:
    ...
    def add_noun_chunks(self, text: str, entities: list[L1Entity] | None = None) -> list[L1Entity]:
...
class L1GlinerComponent(BaseComponent[L1GlinerConfig]):
    ...
    def _setup(self):
        ...
        self.model = GLiNER.from_pretrained(
            self.config.model, token=self.config.token, max_length=self.config.max_length
        )
    ...
    def encode_labels(self, labels: list[str], batch_size: int | None = None) -> torch.Tensor:
        """
    ...
    def extract_entities(self, text: str) -> list[L1Entity]:
        ...
        for ent in raw_entities:
            ...
            left_context, right_context = self._get_context(text, ent["start"], ent["end"])
