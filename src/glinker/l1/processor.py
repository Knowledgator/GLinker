...
from .models import L1Input, L1Config, L1Output, L1GlinerConfig
...
class L1SpacyProcessor(BaseProcessor[L1Config, L1Input, L1Output]):
    ...
    def _default_pipeline(self) -> list[tuple[str, dict[str, Any]]]:
        return [("extract_entities", {}), ("deduplicate", {}), ("sort_by_position", {})]

    def __call__(self, texts: List[str] | None = None, input_data: L1Input = None) -> L1Output:
        ...
        for doc, original_text in zip(
            self.component.nlp.pipe(texts_to_process, batch_size=self.config.batch_size),
            texts_to_process,
        ):
            entities = self._extract_from_doc(doc, original_text)

            pipeline_rest = [
                (method, kwargs) for method, kwargs in self.pipeline if method != "extract_entities"
            ]
...
def create_l1_spacy_processor(config_dict: dict, pipeline: list | None = None) -> L1SpacyProcessor:
...
def create_l1_gliner_processor(
    config_dict: dict, pipeline: list | None = None
) -> L1GlinerProcessor:
