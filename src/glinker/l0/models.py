...
class L0Config(BaseConfig):
    ...
    min_confidence: float = Field(
        0.0, description="Minimum confidence threshold for linked entities"
    )
    include_unlinked: bool = Field(True, description="Include mentions without linked entities")
    return_all_candidates: bool = Field(
        False, description="Return all candidates or only top match"
    )
...
class L0Input(BaseInput):
    ...
    l2_candidates: List[List[DatabaseRecord]] = Field(
        ..., description="Candidates from L2 (per mention)"
    )
...
class L0Entity(BaseOutput):
    ...
    label: str | None = Field(None, description="Entity label from L1")
    ...
    candidates: List[DatabaseRecord] = Field(
        default_factory=list, description="All candidates found in L2 for this mention"
    )
    num_candidates: int = Field(0, description="Number of candidates found")

    # From L3 - entity linking
    linked_entity: LinkedEntity | None = Field(
        None, description="Linked entity if disambiguation was successful"
    )
...
class L0Output(BaseOutput):
    ...
    stats: dict = Field(
        default_factory=dict, description="Pipeline statistics (total, linked, unlinked, etc.)"
    )
